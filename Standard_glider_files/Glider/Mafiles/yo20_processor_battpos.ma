...
<start:b_arg>
    ...
    # DIVE_TO ARGUMENTS
        ...
        # Advanced Flight Controls
            # Ballast Pump Controls
                ...
                b_arg:  d_bpump_value(X)    -400    # 

            # Dive Angle Arguments (using AP)
                b_arg:  d_use_pitch(enum)   	1	# 4  Fluid Pumped absolute
													# 3  Servo on Pitch
													# 2  Pitch, set once from curve
													# 1  BattPos
                b_arg:  d_pitch_value(X)    1  # 23 degrees 26 deg for Nortek 0.454: 23deg for AZFP 0.401
    ...
    # CLIMB_TO ARGUMENTS
        ...
        # Advanced Flight Controls
            # Ballast Pump Controls
                b_arg:  c_use_bpump(enum)    	2	# see d_use_bpump(enum) above
            	b_arg:  c_bpump_value(X)   400	    # Comment in if using autoballast for processor gliders, out for persistor gliders

            # Dive Angle Arguments (using AP)
                b_arg:  c_use_pitch(enum)	1		# see d_use_pitch(enum) above
                b_arg:  c_pitch_value(X)	-1   #  rad 23 degrees
